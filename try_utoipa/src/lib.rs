...
use utoipa::{
    openapi::{Object, RefOr, Schema, SchemaFormat, SchemaType},
    ToSchema,
};
...
impl<'__s> ToSchema<'__s> for Id {
    fn schema() -> (&'__s str, RefOr<Schema>) {
        ...
        object.format = Some(SchemaFormat::Custom("hex".to_owned()));
        object.schema_type = SchemaType::String;
        object.description =
            Some("An Id type representing base for TimelineId and TenantId".to_owned());
        ("Id", RefOr::T(Schema::Object(object)))
    }
}
...
/// like `[173,80,132,115,129,226,72,254,170,201,135,108,199,26,228,24]`.
/// See [`Id`] for alternative ways to serialize it.
#[derive(Clone, Copy, PartialEq, Eq, Hash, PartialOrd, Ord, Serialize, Deserialize, ToSchema)]
